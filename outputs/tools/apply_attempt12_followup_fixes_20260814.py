#!/usr/bin/env python3
"""Run the audited DnT/Tracks/Iron follow-up transaction on Attempt12.
...
impl.PREFLIGHT_REPORT = ROOT / "attempt12-followup-fixes-preflight-20260814.json"
impl.APPLY_REPORT = ROOT / "attempt12-followup-fixes-apply-20260814.json"
impl.POSTVERIFY_REPORT = ROOT / "attempt12-followup-fixes-postverify-20260814.json"


def ensure_attempt12_root_safety() -> None:
    expected = {
    ...
    for value, leaf in expected.items():
        ...
        if resolved.name != leaf:
            raise impl.FollowupError(f"Attempt12 target leaf mismatch: {resolved}")
        try:
        ...
        for runtime_name in ("logs", "crash-reports", impl.GATE_MARKER):
            if (resolved / runtime_name).exists():
                raise impl.FollowupError(f"Attempt12 root already has runtime state: {resolved / runtime_name}")


impl.ensure_root_safety = ensure_attempt12_root_safety
