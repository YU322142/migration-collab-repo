...
# Mechanomania Attempt13 -> Prism 11.0.3 importer.
#
# Safety contract:
...
# - A real import requires the exact, SHA-256-locked Attempt13 PASS gate.
...
$defaultInstanceName = $instanceStem + '-Mechanomania-Matched-Attempt13-NeoForge-1.21.1-20260814'
...
if ([string]::IsNullOrWhiteSpace($Report)) {
    $leaf = if ($PreflightOnly.IsPresent) { 'mechanomania-attempt13-prism-import-preflight-20260814.json' } else { 'mechanomania-attempt13-prism-import-20260814.json' }
...
$requiredModLocks = [ordered]@{
    ...
    'create-carriage-orientation-guard-1.0.0+neoforge.1.21.1-p0.2.jar' = [ordered]@{ bytes = 6288; sha256 = '805D6841BD30B514A059B21BEE4B6C70E183CB379CA286032975DCB961D6D74E' }
    'touhoulittlemaid-1.5.3-neoforge+mc1.21.1.jar' = [ordered]@{ bytes = 24411246; sha256 = '32BE64DD058B7A91F90107972D104BDC0946D858E690D4C72032F64873F9B15B' }
    'yuushya-1.21.0-neoforge-2.3.0-patchouli-safe.1.jar' = [ordered]@{ bytes = 28197402; sha256 = '31DFFD39D1FED94F2088405AF3B8DC862E363BA389015780355571ECCA4A813D' }
    'DnT-ancient-city-overhaul-v2 [NeoForge].jar' = [ordered]@{ bytes = 945155; sha256 = 'A7D3ABB6C39FB50C791D52E596C9D14C22D0287EAF6BA055A687C31C0A4C8A7E' }
    'tracks-neoforge-1.21.1-1.0.1.jar' = [ordered]@{ bytes = 165818; sha256 = '3119FA84955907FD734EF77F2296EC2E546F4442BC3AE13B04046C5D71F61CCF' }
    'irons_spellbooks-1.21.1-3.15.6.jar' = [ordered]@{ bytes = 13584342; sha256 = 'BD8235AEF2F7F4827D8005E9700C1C04E5F3A84C50E0F92685674CAC49E985DB' }
}
...
$excludedRuntimeDirectoryNames = @(
    '.cache', '.mixin.out', '.sable', 'cache', 'crash-reports', 'downloads', 'journeymap', 'logs',
...
function Validate-Mods([string] $Root) {
    ...
    if ($entries.Count -ne $files.Count -or @($files | Where-Object { $_.Extension -ine '.jar' -or (Is-Reparse $_) }).Count -ne 0) { throw 'Attempt13 mods must contain only ordinary active JAR files' }
...
function Inspect-Gate([string] $Path, [string] $Client, [int] $CurrentJarCount, [DateTime] $SourceLatestWriteUtc, [bool] $RequirePass) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        if ($RequirePass) { throw "Attempt13 PASS gate report required before Prism import: $Path" }
        return [ordered]@{ status = 'PENDING'; path = $Path; import_allowed = $false }
    }
    if (-not (Same-Path $Path $expectedGateReport)) { throw "Attempt13 gate report path is not locked: $Path" }
    $gateItem = Get-Item -LiteralPath $Path -Force
    $hash = Sha256 $Path
    if ($hash -cne $expectedGateReportSha256) { throw "Attempt13 gate report fingerprint mismatch: $hash" }
    $gate = Read-Json $Path
    if ([string]$gate.status -cne 'PASS') {
        if ($RequirePass) { throw "Attempt13 gate is not PASS: $($gate.status)" }
        return [ordered]@{ status = [string]$gate.status; path = $Path; sha256 = $hash; import_allowed = $false }
    }
    if (-not (Same-Path ([string]$gate.client) $Client) -or [int]$gate.ports.server -ne 12341 -or [int]$gate.ports.rcon -ne 12342 -or [int]$gate.ports.voice -ne 26341) { throw 'Attempt13 PASS gate identity/ports do not match this client' }
    if ($gate.client_mods.mcmodsync_active -ne $false -or $gate.server_mods.mcmodsync_active -ne $false -or [int]$gate.client_mods.active_jar_count -ne $CurrentJarCount) { throw 'Attempt13 PASS gate mod/MCModSync evidence mismatch' }
    if ($gate.cleanup.all_closed -ne $true -or @($gate.blockers).Count -ne 0 -or [string]::IsNullOrWhiteSpace([string]$gate.completed_at_utc)) { throw 'Attempt13 PASS gate cleanup/completion evidence is incomplete' }
    if ($SourceLatestWriteUtc -gt $gateItem.LastWriteTimeUtc) { throw 'Attempt13 client contains files newer than its PASS gate report' }
...
if (-not (Same-Path $client $expectedClientRoot)) { throw "Attempt13 client root is not the locked D-drive root: $client" }
if (-not (Same-Path $preparePath $expectedPrepareReport)) { throw "Attempt13 prepare report path is not locked: $preparePath" }
if (-not (Same-Path $gatePath $expectedGateReport)) { throw "Attempt13 gate report path is not locked: $gatePath" }
...
if ($ServerAddress -cne '127.0.0.1:12341') { throw 'Attempt13 Prism endpoint must be exactly 127.0.0.1:12341' }
if ($MinMemoryMb -lt 2048 -or $MaxMemoryMb -gt 4096 -or $MaxMemoryMb -lt $MinMemoryMb) { throw 'Attempt13 Prism memory must stay within 2-4 GiB' }
...
if (Is-Reparse (Get-Item -LiteralPath $client -Force)) { throw 'Attempt13 client root may not be a reparse point' }
if (-not (Test-Path -LiteralPath (Join-Path $prism 'portable.txt') -PathType Leaf)) { throw "Not a Prism portable root: $prism" }
if (-not (Test-Path -LiteralPath $preparePath -PathType Leaf) -or (Sha256 $preparePath) -ne $expectedPrepareReportSha256) { throw 'Attempt13 client prepare report fingerprint mismatch' }
...
if (Test-Path -LiteralPath $target) { throw "Refusing to overwrite existing Mechanomania Attempt13 Prism instance: $target" }
if ((Test-Path -LiteralPath $reportPath) -or (Test-Path -LiteralPath $reportSidecar)) { throw 'Refusing to overwrite existing Mechanomania Attempt13 Prism import evidence' }

$prepare = Read-Json $preparePath
if ([string]$prepare.status -cne 'PREPARED' -or -not (Same-Path ([string]$prepare.output_root) $client) -or [string]$prepare.server.address -cne $ServerAddress -or $prepare.server.acceptTextures -ne $false -or [string]$prepare.server.remote_pack -cne 'REJECT' -or [string]$prepare.heap.xms -cne '2G' -or [string]$prepare.heap.xmx -cne '4G' -or $prepare.release.permanent_mod_count_cap -ne $false -or $prepare.safety.java_started -ne $false -or $prepare.safety.prism_started -ne $false) { throw 'Attempt13 client prepare report binding/policy mismatch' }
...
if (-not (Test-Path -LiteralPath $sourceServers -PathType Leaf) -or (Is-Reparse (Get-Item -LiteralPath $sourceServers -Force))) { throw 'Attempt13 servers.dat missing or linked' }
...
if ($PreflightOnly.IsPresent) {
    $value = [ordered]@{
        schema = 1; status = 'PREFLIGHT_PASS'; attempt = 13; generated_at_utc = [DateTime]::UtcNow.ToString('o')
...
$temp = Join-Path $instances ('.mechanomania-attempt13-import-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    if (-not (Same-Snapshot $sourceSnapshot $targetSnapshot) -or -not (Same-Snapshot $sourceSnapshot $sourceAfterCopy)) { throw 'Attempt13 source changed during import or copied snapshot differs' }
    ...
    if ($sourceLatestAfterCopyUtc -gt $gateLastWriteUtc) { throw 'Attempt13 client changed after its PASS gate report' }
    ...
    $value = [ordered]@{
        schema = 1; status = 'IMPORTED_PRISM_INSTANCE'; attempt = 13; imported_at_utc = [DateTime]::UtcNow.ToString('o'); ready_for_manual_launch = $true
    ...
    if ($null -ne $temp -and (Test-Path -LiteralPath $temp -PathType Container) -and (Path-IsWithin $temp $instances) -and ([IO.Path]::GetFileName($temp)).StartsWith('.mechanomania-attempt13-import-', [StringComparison]::Ordinal)) { Remove-Item -LiteralPath $temp -Recurse -Force }
    throw
}
