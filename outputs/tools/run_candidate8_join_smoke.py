#!/usr/bin/env python3
"""Run the Candidate10 client's two-round private-desktop join gate.

The runner is deliberately limited to a disposable Candidate8-series server
and the isolated Candidate10 client rooted under this workspace. It never
...
def validate_paths(target: Path, client_root: Path, report: Path) -> None:
    ...
    external_candidate8 = (
        ...
        and "candidate8" in target_resolved.name.lower()
    )
    if not workspace_server and not external_candidate8:
        raise GateError(
            "target must be outputs/tmp or a disposable candidate8 under migration-audit-work"
        )
    if not is_within(client_root, WORKSPACE_CLIENT_ROOT):
        raise GateError("client root must be under this workspace's outputs/tmp")
...
def candidate8_servers_dat_payload(server_address: str) -> bytes:
...
def configure_disposable_resource_pack_rejection(
    ...
    if required_text == "true":
        raise GateError(
            "Candidate10 policy rejects resource packs, but the server pack is required"
        )
    ...
    expected = candidate8_servers_dat_payload(server_address)
    temporary = path.with_name(f"{path.name}.candidate8.{os.getpid()}.tmp")
    if temporary.exists():
...
def validate_disposable_resource_pack_rejection(
    ...
    *,
    ...
    expected = candidate8_servers_dat_payload(f"127.0.0.1:{server_port}")
    ...
    *,
    ...
    if not destination_matches:
        temporary = destination.with_name(
            f"{destination.name}.candidate8.{os.getpid()}.tmp"
        )
        if temporary.exists():
    ...
    if updated_text != current_text:
        temporary = options_path.with_name(
            f"{options_path.name}.candidate8.{os.getpid()}.tmp"
        )
        if temporary.exists():
...
def normalize_disposable_whitelist(path: Path) -> dict[str, Any]:
    ...
    if value != path.read_text(encoding="utf-8", errors="replace"):
        temporary = path.with_name(path.name + ".candidate8.tmp")
...
class PrivateClientSession:
    ...
    def stop(self) -> dict[str, Any]:
        ...
        self.stop_path.write_text("controlled candidate8 stop\n", encoding="ascii")
        try:
    ...
    def abort(self) -> None:
        try:
            self.stop_path.write_text("abort candidate8 client\n", encoding="ascii")
        except OSError:
            pass
        try:
            self.process.wait(timeout=30)
    ...
    *,
    ...
    try:
        ...
        wait_until(
            ...
            join_timeout,
            f"Candidate10 client round {round_number} join",
...
def execute(args: argparse.Namespace) -> tuple[dict[str, Any], str]:
    ...
    report: dict[str, Any] = {
        "schema": 1,
        "status": "NO_GO",
        "category": "candidate10_private_desktop_join_smoke",
        ...
        report["strict_assertions"] = {
            ...
            "remote_resource_pack_declined": (
                ...
                and all(
                    ...
                    and value["server_properties"]["unchanged"] is True
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Candidate10 client's two-round hidden join gate"
    )
    ...
    parser.add_argument("--teleport-pause-seconds", type=float, default=2.0)
