#!/usr/bin/env python3
"""Run the Candidate13 client's two-round private-desktop join gate.

The runner is deliberately limited to a disposable fresh disposable Candidate13 server
and the isolated Candidate13 client rooted under this workspace. It never
...
SYNTHETIC_USERNAME = "SyntheticOperator"
...
CANDIDATE13_SERVER_MANIFEST = CANDIDATE13_RELEASE_ROOT / "manifests" / "server.json"
...
CANDIDATE13_SERVER_MANIFEST_SHA256 = (
    "FE8771CFAEBCD6E1522ABF27D153D47595DCBDE037BC3A1EE5D1C59156810496"
)
CANDIDATE13_CLIENT_MANIFEST_SHA256 = (
    "261ADB612DB2A2D992F8A8CAC0FC8C753D6620B98B8CB79E693CC434E57216BE"
)
CANDIDATE13_BUNDLE_BUILD_REPORT_SHA256 = (
    "0CFE36F93FACAE0B145866288CE5326736C6BB834C485D25666812DAFADF10E2"
)
CANDIDATE13_PUBLISHED_SERVER_BUNDLE = {
    "files": 52,
    "bytes": 164684183,
    "bundle_sha256": "C1B8326DC78C4D3558C82C6199278F8F95EFB0CE3FCC7A7F9FE107F3FCB7EFD3",
}
CANDIDATE13_RUNTIME_SERVER_BUNDLE = {
    "files": 52,
    "bytes": 164680231,
    "bundle_sha256": "0F06BB7A32527E3B103B048B6545A443C2A78E434E0945E566B7F154F1163A6E",
}
CANDIDATE13_CLIENT_BUNDLE = {
    "files": 52,
    "bytes": 145878089,
    "bundle_sha256": "AC9887DB6F12E0A9E9F8B77030C3F904276DB8BFD4BDF9D01C4B9DAF9EEA4495",
}
CANDIDATE13_BUNDLE_PAIR_SHA256 = (
    "FBE4463D0287FFB8D84EF6A88642C5C87D567009577D8FA39136EF83BBF07D8B"
)
...
WAYPOINT_FILE = "waypoint-fire-equivalence-0.1.1+mc1.21.1.jar"
WAYPOINT_SHA256 = "86A85C0447315AC17D373E3708425CEB8450D9D0CB1FD9C7ABDC82CE8D8E5B92"
RESOURCE_OVERLAY_FILE = "migration-resource-overlay-1.2.0+mc1.21.1-candidate13.jar"
RESOURCE_OVERLAY_SHA256 = "BCCB7D7CF8019D8895A081D563E578712D7CDF93DA0AD9EAFB31067439C62862"
CC_COMPUTER_ID = 11
CC_COMPUTER_POSITION = (1403, 67, -5088)
CC_COMPUTER_BLOCK_ENTITY_ID = "computercraft:computer_normal"
ATTEMPT_MARKER_NAME = ".candidate13-join-gate-attempt.json"
REQUIRED_LOCAL_RESOURCE_PACK_SOURCE = Path(
    ROOT / "outputs" / "candidate13-resource-closure-20260812" /
    "\u4e16\u754c\u6307\u5b9a\u8d44\u6e90\u5305\u55b5-mc1.21.1-candidate13.zip"
)
...
REQUIRED_LOCAL_RESOURCE_PACK_BYTES = 110377999
...
SERVER_START_RE = re.compile(r"Starting minecraft server version\s+1\.21\.1")
...
def validate_paths(target: Path, client_root: Path, report: Path) -> None:
    ...
    if not external_pipeline_target:
        raise GateError(f"target must be the locked fresh Candidate13 runtime: {PIPELINE_PREPARED_TARGET}")
    if client_root.resolve() != CANDIDATE13_CLIENT_ROOT.resolve():
        raise GateError(
            f"client root must be the fresh locked Candidate13 root: {CANDIDATE13_CLIENT_ROOT}"
        )
    if not client_root.is_dir() or client_root.is_symlink():
        raise GateError("Candidate13 client root is missing, linked, or not a directory")
...
def candidate13_servers_dat_payload(server_address: str) -> bytes:
...
def configure_disposable_resource_pack_rejection(
    ...
    if required_text == "true":
        raise GateError(
            "Candidate13 policy rejects resource packs, but the server pack is required"
        )
    if not client_root.is_dir() or client_root.is_symlink():
    ...
    expected = candidate13_servers_dat_payload(server_address)
    temporary = path.with_name(f"{path.name}.candidate13.{os.getpid()}.tmp")
    if temporary.exists():
...
def validate_disposable_resource_pack_rejection(
    ...
    *,
    ...
    expected = candidate13_servers_dat_payload(f"127.0.0.1:{server_port}")
...
def configure_local_world_resource_pack(
    ...
    *,
    ...
    """Copy and enable the already-derived, hash-locked Candidate13 local pack."""
    if not client_root.is_dir() or client_root.is_symlink():
        raise GateError(f"client root is missing or linked: {client_root}")
    ...
    if source_evidence["pack_format"] != MINECRAFT_1_21_1_RESOURCE_PACK_FORMAT:
        raise GateError("locked Candidate13 local pack is not format 34")
    ...
    destination_matches = (
        ...
        and destination_before["bytes"] == source_artifact["bytes"]
        and destination_before["sha256"] == source_artifact["sha256"]
    )
    ...
    if not destination_matches:
        temporary = destination.with_name(
            f"{destination.name}.candidate13.{os.getpid()}.tmp"
        )
        if temporary.exists():
            raise GateError(f"refusing to reuse temporary resource pack: {temporary}")
        try:
            shutil.copy2(source, temporary)
            temporary_artifact = file_artifact(temporary)
            if temporary_artifact["sha256"] != expected_sha256 or temporary_artifact["bytes"] != expected_bytes:
                raise GateError("Candidate13 local pack copy failed byte validation")
            temporary_evidence = resource_pack_zip_evidence(temporary, expected_zip_entries)
            if temporary_evidence != source_evidence:
                raise GateError("Candidate13 local pack copy failed ZIP validation")
    ...
    if updated_text != current_text:
        temporary = options_path.with_name(
            f"{options_path.name}.candidate13.{os.getpid()}.tmp"
        )
        if temporary.exists():
    ...
    if (
        source_after["bytes"] != source_artifact["bytes"]
        or source_after["sha256"] != source_artifact["sha256"]
    ):
    ...
    if (
        destination_after["bytes"] != source_artifact["bytes"]
        or destination_after["sha256"] != source_artifact["sha256"]
        or destination_evidence != source_evidence
    ):
        raise GateError("Candidate13 local world resource pack content binding mismatch")
    ...
    return {
        "policy": "copy_locked_candidate13_derived_pack_and_enable",
        "source": {
        ...
        "locked_derived_pack": {
            "already_1_21_1_compatible": True,
...
def normalize_disposable_whitelist(path: Path) -> dict[str, Any]:
    ...
    if value != path.read_text(encoding="utf-8", errors="replace"):
        temporary = path.with_name(path.name + ".candidate13.tmp")
...
def validate_prepare_report(
    ...
    if (
        ...
        or "prepare" not in resolved_report.name.lower()
    ):
        raise GateError("prepare report must be an external, Candidate13-bound disposable target report")
    ...
    manifest = value.get("resource_sanitization", {}).get("runtime_mod_manifest")
    observed_runtime_bundle = {
        ...
        "bundle_sha256": manifest.get("bundle_sha256") if isinstance(manifest, dict) else None,
    }
    if observed_runtime_bundle != CANDIDATE13_RUNTIME_SERVER_BUNDLE:
        raise GateError("prepare report sanitized runtime bundle mismatch")
    changes = value.get("resource_sanitization", {}).get("changes")
    jar_changes = {
    ...
    if set(jar_changes) != {
        ...
        raise GateError(f"prepare report runtime JAR sanitizer scope mismatch: {sorted(jar_changes)}")
    return {
        "path": str(path.resolve()),
        "bytes": path.stat().st_size,
        "sha256": sha256_file(path),
        "status": value.get("status"),
        "runtime_server_bundle": observed_runtime_bundle,
        "sanitized_jars": sorted(jar_changes),
    }
...
def validate_client_prepare_report(
    ...
    if path.resolve() != CANDIDATE13_CLIENT_PREPARE_REPORT.resolve():
        raise GateError("client prepare report must be the locked Candidate13 preparation report")
    if forbidden_source_path(path):
        raise GateError("client prepare report may not come from the historical source")
    value = read_json_object(path, "Candidate13 client prepare report")
    if value.get("schema") != 1 or value.get("status") != "PREPARED" or value.get("candidate") != 13:
        raise GateError("client prepare report is not Candidate13 PREPARED")
    ...
    if value.get("java_started") is not False or value.get("prism_started") is not False:
        raise GateError("client preparation report indicates a runtime launch")
    release = value.get("release")
    if not isinstance(release, dict):
        raise GateError("client prepare report has no release binding")
    if (
        Path(str(release.get("root", ""))).resolve() != CANDIDATE13_RELEASE_ROOT.resolve()
        or str(release.get("ready_sha256", "")).upper() != CANDIDATE13_READY_SHA256
        or str(release.get("release_lock_sha256", "")).upper() != CANDIDATE13_READY_SHA256
        or str(release.get("client_manifest_sha256", "")).upper() != CANDIDATE13_CLIENT_MANIFEST_SHA256
        or str(release.get("client_bundle_sha256", "")).upper() != CANDIDATE13_CLIENT_BUNDLE["bundle_sha256"]
        or int(release.get("file_count", -1)) != CANDIDATE13_CLIENT_BUNDLE["files"]
    ):
        raise GateError("client prepare report Candidate13 release binding mismatch")
    allowed_top_level = {"assets", "config", "data", "defaultconfigs", "libraries", "mods", "natives", "options.txt", "resourcepacks", "servers.dat", "versions"}
    actual_top_level = {item.name for item in client_root.iterdir()}
    if not allowed_top_level.issubset(actual_top_level):
        raise GateError(f"client root is incomplete: missing={sorted(allowed_top_level - actual_top_level)}")
    for directory_name in ("mods", "resourcepacks", "natives"):
        directory = client_root / directory_name
        if not directory.is_dir() or directory.is_symlink():
            raise GateError(f"client root has unsafe {directory_name} directory")
    options_path = client_root / "options.txt"
    if not options_path.is_file() or options_path.is_symlink():
        raise GateError("client options.txt is missing or linked")
    if value.get("local_resource_pack", {}).get("sha256") != REQUIRED_LOCAL_RESOURCE_PACK_SHA256:
        raise GateError("client prepare report local resource pack hash mismatch")
    server_binding = value.get("server")
    if not isinstance(server_binding, dict) or int(server_binding.get("port", -1)) != 12341 or server_binding.get("accept_remote_resource_pack") is not False or server_binding.get("servers_dat_acceptTextures") is not False:
        raise GateError("client prepare report remote resource-pack policy mismatch")
    actual = bundle_binding(client_root / "mods")
    expected = {"files": client_bundle["files"], "bytes": client_bundle["bytes"], "bundle_sha256": client_bundle["bundle_sha256"]}
    require_exact_bundle(actual, expected, "Candidate13 client prepared")
    for filename, digest in ((WAYPOINT_FILE, WAYPOINT_SHA256), (CC_GUARD_FILE, CC_GUARD_SHA256), (CREATE_GUARD_FILE, CREATE_GUARD_SHA256), (RESOURCE_OVERLAY_FILE, RESOURCE_OVERLAY_SHA256)):
        artifact = file_artifact(client_root / "mods" / filename)
        if artifact["sha256"] != digest:
            raise GateError(f"Candidate13 client guard/overlay mismatch: {filename}")
    return {
        "path": str(path.resolve()),
        "bytes": path.stat().st_size,
        "sha256": sha256_file(path),
        "status": value.get("status"),
        "identity": {"username": SYNTHETIC_USERNAME, "uuid": SYNTHETIC_UUID, "permission_level": SYNTHETIC_PERMISSION_LEVEL},
        "bundle_sha256": client_bundle["bundle_sha256"],
        "local_resource_pack": value.get("local_resource_pack"),
...
def command_plan(username: str = SYNTHETIC_USERNAME) -> list[dict[str, Any]]:
    ...
    fixture = "candidate13_waypoint_fixture"
    commands.extend(
        (
            {
                "kind": "waypoint_fixture",
                "site": None,
                "command": (
                    f'execute at {username} run summon minecraft:armor_stand ~ ~ ~ '
                    f'{{Tags:["{fixture}"],NoGravity:1b,Invisible:1b}}'
                ),
            },
            {
                "kind": "waypoint_attribute",
                "site": None,
                "command": (
                    f"attribute @e[tag={fixture},limit=1] "
                    "minecraft:waypoint_transmit_range base set 64"
                ),
            },
            {
                "kind": "waypoint_hex",
                ...
                "hex": "0F0",
                "command": (
                    f"execute as {username} run waypoint modify "
                    f"@e[tag={fixture},limit=1] color hex 0F0"
                ),
            },
            {
                "kind": "waypoint_color_probe",
                "site": None,
                "expected_color": 65280,
                "command": f"data get entity @e[tag={fixture},limit=1] locator_bar_icon",
            },
            {
                "kind": "waypoint_hex",
                "site": None,
                ...
                "command": (
                    f"execute as {username} run waypoint modify "
                    f"@e[tag={fixture},limit=1] color hex FF00AA"
                ),
            },
            {
                "kind": "waypoint_color_probe",
                "site": None,
                "expected_color": 16711850,
                "command": f"data get entity @e[tag={fixture},limit=1] locator_bar_icon",
            },
            {
                "kind": "waypoint_fixture_cleanup",
                "site": None,
                "command": f"kill @e[tag={fixture}]",
            },
            {"kind": "save", "site": None, "command": "save-all flush"},
        )
    )
    return commands
...
def validate_waypoint_command_evidence(commands: list[dict[str, Any]]) -> dict[str, Any]:
    probes = [row for row in commands if row.get("kind") == "waypoint_color_probe"]
    expected = [65280, 16711850]
    observed = [int(row.get("expected_color", -1)) for row in probes]
    if observed != expected:
        raise GateError(f"Waypoint color-probe sequence mismatch: {observed}")
    for row in probes:
        color = int(row["expected_color"])
        if str(color) not in str(row.get("response", "")):
            raise GateError(
                f"Waypoint color {color} was not persisted: {row.get('response', '')}"
            )
    hex_rows = [row for row in commands if row.get("kind") == "waypoint_hex"]
    if [row.get("hex") for row in hex_rows] != ["0F0", "FF00AA"]:
        raise GateError("Waypoint hex command sequence mismatch")
    return {
        "permission_level": SYNTHETIC_PERMISSION_LEVEL,
        "hex_values": ["0F0", "FF00AA"],
        "persisted_colors": expected,
        "validated": True,
    }
...
class ServerSession:
    ...
    def wait_ready(self, startup_timeout: float, bootstrap_timeout: float) -> None:
        bootstrap_started = time.monotonic()
        wait_until(
            lambda: (
                self.current_log() != self.previous_log
                and bool(SERVER_START_RE.search(self.current_log()))
            ),
            bootstrap_timeout,
            f"server round {self.round_number} NeoForge bootstrap",
            health=self.assert_alive,
        )
        dedicated_started = time.monotonic()
        wait_until(
            lambda: bool(DONE_RE.search(self.current_log())),
            startup_timeout,
            f"server round {self.round_number} dedicated startup Done",
            health=self.assert_alive,
        )
        self.startup_timing = {
            "bootstrap_seconds": round(dedicated_started - bootstrap_started, 3),
            "dedicated_startup_seconds": round(time.monotonic() - dedicated_started, 3),
            "bootstrap_timeout_seconds": bootstrap_timeout,
            "dedicated_startup_timeout_seconds": startup_timeout,
        }
...
class PrivateClientSession:
    ...
    def stop(self) -> dict[str, Any]:
        ...
        self.stop_path.write_text("controlled candidate13 stop\n", encoding="ascii")
        try:
    ...
    def abort(self) -> None:
        try:
            self.stop_path.write_text("abort candidate13 client\n", encoding="ascii")
        except OSError:
            pass
        try:
            self.process.wait(timeout=30)
...
def validate_candidate13_release() -> dict[str, Any]:
    paths = {
        "ready": (CANDIDATE13_READY, CANDIDATE13_READY_SHA256),
        "release_lock": (CANDIDATE13_RELEASE_LOCK, CANDIDATE13_READY_SHA256),
        "server_manifest": (CANDIDATE13_SERVER_MANIFEST, CANDIDATE13_SERVER_MANIFEST_SHA256),
        "client_manifest": (CANDIDATE13_CLIENT_MANIFEST, CANDIDATE13_CLIENT_MANIFEST_SHA256),
        "bundle_build_report": (CANDIDATE13_BUNDLE_BUILD_REPORT, CANDIDATE13_BUNDLE_BUILD_REPORT_SHA256),
    }
    artifacts: dict[str, Any] = {}
    for name, (path, expected_hash) in paths.items():
        artifact = file_artifact(path)
        if artifact["sha256"] != expected_hash:
            raise GateError(f"Candidate13 {name} hash mismatch")
        artifacts[name] = artifact
    if CANDIDATE13_READY.read_bytes() != CANDIDATE13_RELEASE_LOCK.read_bytes():
        raise GateError("Candidate13 READY and release-lock are not byte-identical")
    ready = read_json_object(CANDIDATE13_READY, "Candidate13 READY")
    server_manifest = read_json_object(CANDIDATE13_SERVER_MANIFEST, "Candidate13 server manifest")
    client_manifest = read_json_object(CANDIDATE13_CLIENT_MANIFEST, "Candidate13 client manifest")
    build = read_json_object(CANDIDATE13_BUNDLE_BUILD_REPORT, "Candidate13 bundle build report")
    if (
        ready.get("schema") != 1
        or ready.get("candidate") != 13
        or ready.get("status") != "PASS"
        or ready.get("source_unchanged") is not True
        or Path(str(ready.get("output_root", ""))).resolve() != CANDIDATE13_RELEASE_ROOT.resolve()
        or ready.get("bundle_pair_sha256") != CANDIDATE13_BUNDLE_PAIR_SHA256
    ):
        raise GateError("Candidate13 READY identity mismatch")
    for side, manifest, expected_hash, expected_bundle in (
        ("server", server_manifest, CANDIDATE13_SERVER_MANIFEST_SHA256, CANDIDATE13_PUBLISHED_SERVER_BUNDLE),
        ("client", client_manifest, CANDIDATE13_CLIENT_MANIFEST_SHA256, CANDIDATE13_CLIENT_BUNDLE),
    ):
        release_side = ready.get(side)
        release_observed = {
            "files": release_side.get("file_count") if isinstance(release_side, dict) else None,
            "bytes": release_side.get("bytes") if isinstance(release_side, dict) else None,
            "bundle_sha256": release_side.get("bundle_sha256") if isinstance(release_side, dict) else None,
        }
        if (
            release_observed != expected_bundle
            or release_side.get("manifest_sha256") != expected_hash
            or manifest.get("schema") != 1
            or manifest.get("candidate") != 13
            ...
            raise GateError(f"Candidate13 {side} release/manifest mismatch")
    policy = ready.get("runtime_sanitization_policy")
    if (
        not isinstance(policy, dict)
        or policy.get("published_bundle_state") != "unsanitized"
        or policy.get("waypoint_jar_must_remain_byte_identical") is not True
        or policy.get("client_runtime_jar_transforms_allowed") is not False
    ):
        raise GateError("Candidate13 runtime sanitization boundary mismatch")
    resource = ready.get("resource_closure", {}).get("local_resource_pack", {})
    if (
        resource.get("sha256") != REQUIRED_LOCAL_RESOURCE_PACK_SHA256
        or resource.get("bytes") != REQUIRED_LOCAL_RESOURCE_PACK_BYTES
        or resource.get("entries") != REQUIRED_LOCAL_RESOURCE_PACK_ZIP_ENTRIES
    ):
        raise GateError("Candidate13 READY local resource-pack binding mismatch")
    if (
        build.get("status") != "PASS"
        or Path(str(build.get("output_root", ""))).resolve() != CANDIDATE13_RELEASE_ROOT.resolve()
        or build.get("ready_sha256") != CANDIDATE13_READY_SHA256
        or build.get("bundle_pair_sha256") != CANDIDATE13_BUNDLE_PAIR_SHA256
        or build.get("server_bundle_sha256") != CANDIDATE13_PUBLISHED_SERVER_BUNDLE["bundle_sha256"]
        or build.get("client_bundle_sha256") != CANDIDATE13_CLIENT_BUNDLE["bundle_sha256"]
        or build.get("overlay_sha256") != RESOURCE_OVERLAY_SHA256
    ):
        raise GateError("Candidate13 bundle build report binding mismatch")
    return {
        **artifacts,
        "bundle_pair_sha256": CANDIDATE13_BUNDLE_PAIR_SHA256,
        "published_server_bundle": CANDIDATE13_PUBLISHED_SERVER_BUNDLE,
        "expected_runtime_server_bundle": CANDIDATE13_RUNTIME_SERVER_BUNDLE,
        "client_bundle": CANDIDATE13_CLIENT_BUNDLE,
        "resource_pack": resource,
    }


def validate_candidate13_runtime_bundles(
    target: Path,
    client_root: Path,
    ...
    require_exact_bundle(
        server_bundle, CANDIDATE13_RUNTIME_SERVER_BUNDLE, "Candidate13 server runtime"
    )
    require_exact_bundle(client_bundle, CANDIDATE13_CLIENT_BUNDLE, "Candidate13 client")
    guards: dict[str, Any] = {}
    for filename, digest in (
        (CC_GUARD_FILE, CC_GUARD_SHA256),
        (CREATE_GUARD_FILE, CREATE_GUARD_SHA256),
        (WAYPOINT_FILE, WAYPOINT_SHA256),
        (RESOURCE_OVERLAY_FILE, RESOURCE_OVERLAY_SHA256),
    ):
        ...
        if server["sha256"] != digest or client["sha256"] != digest:
            raise GateError(f"Candidate13 guard changed at runtime: {filename}")
        guards[filename] = {
...
def claim_fresh_gate_attempt(target: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise GateError(
            "Candidate13 target already has a gate-attempt marker; failed or used worlds "
...
def run_round(
    *,
    ...
    try:
        ...
        server.wait_ready(startup_timeout, bootstrap_timeout)
        ...
        wait_until(
            ...
            join_timeout,
            f"Candidate13 client round {round_number} join",
        ...
        waypoint_evidence = validate_waypoint_command_evidence(commands)
        ...
        return {
            ...
            "startup_timing": server.startup_timing,
...
def execute(args: argparse.Namespace) -> tuple[dict[str, Any], str]:
    ...
    report: dict[str, Any] = {
        "schema": 1,
        "status": "NO_GO",
        "category": "candidate13_private_desktop_join_smoke",
        "generated_at_utc": utc_now(),
        "target": str(target),
    ...
    try:
        ...
        release_binding = validate_candidate13_release()
        server_bundle = bundle_binding(target / "mods")
        client_bundle = bundle_binding(client_root / "mods")
        runtime_bundle_binding = validate_candidate13_runtime_bundles(
        ...
        report["bindings"] = {
            "candidate13_release": release_binding,
            ...
            "runtime": {
                ...
                "candidate13_bundles": runtime_bundle_binding,
            },
        ...
        report["strict_assertions"] = {
            ...
            "each_round_waypoint_op": all(
                value.get("waypoint_op", {}).get("validated") is True
                and value.get("waypoint_op", {}).get("permission_level") == 4
                and value.get("waypoint_op", {}).get("persisted_colors")
                == [65280, 16711850]
                for value in report["rounds"]
            ),
            ...
            "candidate13_exact_bundles": runtime_bundle_binding[
        ...
        if (
            ...
            or not all(
                ...
                for key in (
                    ...
                    "each_round_waypoint_op",
                    "computer_11_on_preserved",
                    "candidate13_exact_bundles",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Candidate13 client's two-round hidden join gate"
    )
    parser.add_argument("--target", type=Path, default=PIPELINE_PREPARED_TARGET)
    parser.add_argument("--client-root", type=Path, default=CANDIDATE13_CLIENT_ROOT)
    parser.add_argument("--prepare-report", type=Path, default=CANDIDATE13_PREPARE_REPORT)
    parser.add_argument("--client-prepare-report", type=Path, default=CANDIDATE13_CLIENT_PREPARE_REPORT)
    parser.add_argument(
        "--report",
        type=Path,
        default=OUTPUTS / "candidate13-join-gate-r2-20260812.json",
    )
