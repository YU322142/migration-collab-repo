...
AUDIT_ROOT = pathlib.Path(r"<AUDIT_ROOT>").resolve()
PRODUCTION_ROOT = pathlib.Path(r"<TRANS_ROOT>\20260807").resolve()
...
def build_inventory() -> dict[str, Any]:
    ...
    candidates = [
        item(
            ...
            r"<AUDIT_ROOT>\mod-candidates\barched-neoforge-0.0.12-migration.1-asm.jar",
            "conditional-candidate",
            "已在服务端冒烟中使用；必须与 Architectury 13.0.11 和 Cloth Config 15.0.140 配套。真实客户端 GUI/渲染门禁仍未关闭。",
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\barched-neoforge-0.0.12-migration.1-asm.jar"],
            ["outputs/barched-migration-audit-20260808.md"],
        ),
        item(
            "Chest Colorizer", "candidate", "client-only",
            r"<AUDIT_ROOT>\mod-candidates\chest-colorizer-1.6.1-equivalence.2+mc1.21.1-neoforge.jar",
            "conditional-candidate",
        ...
        item(
            ...
            r"<AUDIT_ROOT>\create-dynamic-blocking-neoforge\build\libs\create-dynamic-blocking-1.0.0+neoforge.1.21.1-equivalence.1.jar",
            "conditional-candidate",
            "Create 6.0.10 API/Mixin/config/命令和真实对象 fixture 已通过；多 tick 双列车、真实客户端冷启动与全栈冲突仍需验证。",
            [r"<AUDIT_ROOT>\create-dynamic-blocking-smoke1\mods\create-dynamic-blocking-1.0.0+neoforge.1.21.1-equivalence.1.jar"],
            ["outputs/create-dynamic-blocking-equivalence-audit-20260809.md"],
        ),
        item(
            "Create NERFAD", "candidate", "server+client",
            r"<AUDIT_ROOT>\mod-candidates\create-nerfad-1.2.3-neoforge+mc1.21.1.jar",
            "conditional-candidate",
            "16/16 配方结构、冷启动和 reload hash 通过；实际机器处理、JEI 与最终全栈仍需验证。",
            [r"<AUDIT_ROOT>\create-6.0.10-smoke3\mods\create_nerfad-1-2-3.jar"],
            ["outputs/create-nerfad-equivalence-audit-20260808.md"],
        ),
        item(
            "Happy Ghast", "candidate", "server+client",
            r"<AUDIT_ROOT>\HappyGhast-1.21.1-equivalence\build\libs\happyghast-equivalence-1.0.0-equivalence.1+mc1.21.1.jar",
            "conditional-candidate",
            "NeoForge 两次启动、home_pos/home_radius NBT 往返和静态 client hook 通过；第三人称相机、temptation/player 客户端门禁仍在。",
            [r"<AUDIT_ROOT>\happyghast-equivalence-smoke1\mods\happyghast-equivalence-1.0.0-equivalence.1+mc1.21.1.jar", r"<AUDIT_ROOT>\nautilus-equivalence-smoke2\mods\happyghast-equivalence-1.0.0-equivalence.1+mc1.21.1.jar"],
            ["<AUDIT_ROOT>/HappyGhast-1.21.1-equivalence/SMOKE-20260808.md"],
        ),
        item(
            "Kaleidoscope Cookery", "candidate", "server+client",
            r"<AUDIT_ROOT>\KaleidoscopeCookery-1.21.1-neoforge\build\libs\kaleidoscopecookery-1.4.1.7-migration.3-neoforge+mc1.21.1.jar",
            "conditional-candidate",
            "migration.3 是最新构建；配方/交易/官方 1.21.1 fixture 通过，生产 chef UUID/accounting 和真实客户端仍需关闭。",
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscopecookery-1.4.1.7-migration.3-neoforge+mc1.21.1.jar", r"<AUDIT_ROOT>\tavern-migration-smoke1\mods\kaleidoscopecookery-1.4.1.7-migration.3-neoforge+mc1.21.1.jar"],
        ...
        item(
            ...
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscope_end-1.0.14-migration.7-neoforge+mc1.21.1.jar", r"<AUDIT_ROOT>\end-official-upgrade-smoke1\mods\kaleidoscope_end-1.0.14-migration.7-neoforge+mc1.21.1.jar"],
            ["outputs/end-official-upgrade-smoke-20260808.md"],
        ),
        item(
            "Kaleidoscope Nether equivalence overlay", "candidate", "server+client",
            r"<AUDIT_ROOT>\KaleidoscopeNether-1.21.1-equivalence\build\libs\kaleidoscope-nether-equivalence-backport-1.1.9-equivalence.3+mc1.21.1.jar",
            "conditional-candidate",
            "这是 Nether 等价 overlay（含 Froglight 覆盖），不是独立 Froglight JAR；依赖 Nether 1.1.2、Cookery migration.3、Tavern 1.2.0。",
            [r"<AUDIT_ROOT>\tavern-migration-smoke1\mods\kaleidoscope-nether-equivalence-backport-1.1.9-equivalence.3+mc1.21.1.jar", r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscope-nether-equivalence-backport-1.1.9-equivalence.3+mc1.21.1.jar"],
            ["outputs/froglight-equivalence-audit-20260808.md"],
        ),
        item(
            "Kaleidoscope Tavern", "candidate", "server+client",
            r"<AUDIT_ROOT>\KaleidoscopeTavern-1.21.1\build\libs\kaleidoscopetavern-1.2.0-neoforge+mc1.21.1.jar",
            "conditional-candidate",
            "build/libs 版本是最新候选；world-migration-smoke1 仍有旧字节副本，必须替换后再做最终全栈和客户端验证。",
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscopetavern-1.2.0-neoforge+mc1.21.1.jar", r"<AUDIT_ROOT>\tavern-migration-smoke1\mods\kaleidoscopetavern-1.2.0-neoforge+mc1.21.1.jar"],
            ["outputs/tavern-migration-audit-20260808.md"],
        ),
        item(
            "MineAstr", "candidate", "server+client",
            r"<AUDIT_ROOT>\mineastr-validation\neoforge-1.21.1-current\build\libs\mineastr-0.6.25.jar",
            "conditional-candidate",
            "0.6.25 是当前 NeoForge 候选；静态/服务端 gate 通过，真实客户端 GUI/网络、AstrBot bridge、权限和客户端 TOML 迁移仍未关闭。",
            [r"<AUDIT_ROOT>\mineastr-validation\smoke1\mods\mineastr-0.6.25.jar"],
        ...
        item(
            ...
            r"<AUDIT_ROOT>\MishangUC-1.21.1-equivalence\build\libs\mishanguc-pale-oak-equivalence-backport-1.6.3-beta.5-equivalence.1+mc1.21.1.jar",
            "conditional-candidate",
            "Overlay 候选可用，但基础 mishanguc 仍是 Fabric JAR，经 Connector 运行；Connector 生产稳定性和真实客户端仍是门禁。",
            [r"<AUDIT_ROOT>\mishang-equivalence-smoke1\mods\mishanguc-pale-oak-equivalence-backport-1.6.3-beta.5-equivalence.1+mc1.21.1.jar"],
        ...
        item(
            ...
            r"<AUDIT_ROOT>\Potted-Farms-1.21.1-equivalence\potted-farms-1.1.1-equivalence3.jar",
            "conditional-candidate",
        ...
        item(
            ...
            r"<AUDIT_ROOT>\respawn-pitch-compat\mod\build\libs\respawn-pitch-compat-1.0.0+mc1.21.1.jar",
            "conditional-candidate",
            "静态/NBT/API 通过；真实玩家死亡/重生、客户端 locale 和 HUD 仍需验证。",
            [r"<AUDIT_ROOT>\respawn-pitch-compat\smoke-server\mods\respawn-pitch-compat-1.0.0+mc1.21.1.jar"],
            ["outputs/respawn-pitch-equivalence-audit-20260809.md"],
        ),
        item(
            "Resource Error Overlay", "candidate", "server+client",
            r"<AUDIT_ROOT>\mod-candidates\migration-resource-overlay-1.1.0+mc1.21.1.jar",
            "conditional-candidate",
            "资源错误覆盖层候选；静态资源检查通过，最终全栈需使用最新 JAR 再验证。",
            [r"<AUDIT_ROOT>\resource-overlay-fullstack-smoke1\mods\migration-resource-overlay-1.0.0+mc1.21.1.jar"],
        ...
        item(
            ...
            r"<AUDIT_ROOT>\Toms-Storage-NeoForge-1.21.1-perf-port\NeoForge\build\libs\toms_storage-neoforge-1.21.1-2.4.1-perf5.2.jar",
            "conditional-candidate",
            "perf5.2 候选包含 2.9.2 优化算法/7 filter MCA 转换；Create 6.0.10 空服 smoke 通过，完整世界转换、客户端和全栈仍需验证。",
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\toms_storage-neoforge-1.21.1-2.4.1-perf5.2.jar", r"<AUDIT_ROOT>\tavern-migration-smoke1\mods\toms_storage-neoforge-1.21.1-2.4.1-perf5.2.jar"],
            ["<AUDIT_ROOT>/Toms-Storage-NeoForge-1.21.1-perf-port/MIGRATION-1.21.11-TO-1.21.1.md"],
        ),
        item(
            ...
            [r"<AUDIT_ROOT>\world-migration-smoke1\mods\waypoint-fire-equivalence-0.1.0-draft+mc1.21.1.jar", r"<AUDIT_ROOT>\waypoint-fire-equivalence\smoke-server\mods\waypoint-fire-equivalence-0.1.0-draft+mc1.21.1.jar"],
    ...
    support = [
        item(
            ...
            r"<AUDIT_ROOT>\mod-candidates\mishanguc-1.6.1-1.21.1-migration.1.jar",
            "connector-dependent",
            ...
            [r"<AUDIT_ROOT>\mishang-equivalence-smoke1\mods\mishanguc-1.6.1-1.21.1.jar"],
        ),
        item("Content Backport", "support", "server+client", r"<AUDIT_ROOT>\world-migration-smoke1\mods\backport-1.5.jar", "support-ready", "Happy Ghast、NERFAD、Tavern、Mishang/Potted 的 1.21.1 内容支持。", [r"<AUDIT_ROOT>\create-dynamic-blocking-smoke1\mods\backport-1.5.jar"]),
        item("Create", "support", "server+client", r"<AUDIT_ROOT>\world-migration-smoke1\mods\create-1.21.1-6.0.10.jar", "support-ready", "Create 6.0.10 隔离服基线。", [r"<AUDIT_ROOT>\create-dynamic-blocking-smoke1\mods\create-1.21.1-6.0.10.jar"]),
        item("Architectury", "support", "server+client", r"<AUDIT_ROOT>\world-migration-smoke1\mods\architectury-13.0.11-neoforge.jar", "support-ready", "Barched 和多个客户端/共享组件依赖。", [r"<AUDIT_ROOT>\happyghast-equivalence-smoke1\mods\architectury-13.0.11-neoforge.jar"]),
        item("Cloth Config", "support", "server+client", r"<AUDIT_ROOT>\world-migration-smoke1\mods\cloth-config-15.0.140-neoforge.jar", "support-ready", "Barched 配置界面依赖。", [r"<AUDIT_ROOT>\happyghast-equivalence-smoke1\mods\cloth-config-15.0.140-neoforge.jar"]),
        item("Kaleidoscope Nether base", "support", "server+client", r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscope_nether-1.1.2-neoforge+mc1.21.1.jar", "support-ready", "Nether equivalence overlay 的明确 base 版本。", [r"<AUDIT_ROOT>\tavern-migration-smoke1\mods\kaleidoscope_nether-1.1.2-neoforge+mc1.21.1.jar"]),
        item("Mishang UC base", "support", "server+client (Fabric via Connector)", r"<AUDIT_ROOT>\world-migration-smoke1\mods\mishanguc-1.6.1-1.21.1.jar", "connector-dependent", "基础 JAR 是 Fabric 模组，需 Connector；不是原生 NeoForge 适配。", [r"<AUDIT_ROOT>\mishang-equivalence-smoke1\mods\mishanguc-1.6.1-1.21.1.jar"]),
        item("GriefLogger", "replacement", "server-only", r"<AUDIT_ROOT>\world-migration-smoke1\mods\grieflogger-1.2.10-1.21.1-neoforge.jar", "replacement-ready", "按用户决定替代 Ledger；不迁移 Ledger 历史、查询和回滚数据，属于明确功能/数据取舍。", [r"<AUDIT_ROOT>\grieflogger-smoke1\mods\grieflogger-1.2.10-1.21.1-neoforge.jar"]),
    ]

    support = [entry for entry in support if entry["component"] != "Mishang UC base"]

    stale = [
        stale_item("XiyusLogin migration2", r"<AUDIT_ROOT>\final-client-mods-candidate3\xiyuslogin-1.4-migration2.jar", "rejected: bcrypt's transitive at.favre.lib:bytes runtime dependency is missing; live synthetic registration failed; use migration4"),
        stale_item("Barched invalid all artifact", r"<AUDIT_ROOT>\Barched-source\neoforge\build\libs\barched-neoforge-0.0.12-migration.1-all.jar", "22-byte manifestless Gradle artifact; do not deploy; use tested -asm runtime candidate."),
        stale_item("Kaleidoscope Cookery migration.1", r"<AUDIT_ROOT>\KaleidoscopeCookery-1.21.1-neoforge\build\libs\kaleidoscopecookery-1.4.1.7-migration.1-neoforge+mc1.21.1.jar", "older than migration.3"),
        stale_item("Kaleidoscope Cookery migration.2", r"<AUDIT_ROOT>\KaleidoscopeCookery-1.21.1-neoforge\build\libs\kaleidoscopecookery-1.4.1.7-migration.2-neoforge+mc1.21.1.jar", "older than migration.3"),
        stale_item("Potted Farms equivalence2 in world smoke", r"<AUDIT_ROOT>\world-migration-smoke1\mods\potted-farms-1.1.1-equivalence2.jar", "rejected; replace with equivalence3 candidate"),
        stale_item("Kaleidoscope Tavern world-smoke copy", r"<AUDIT_ROOT>\world-migration-smoke1\mods\kaleidoscopetavern-1.2.0-neoforge+mc1.21.1.jar", "stale byte copy; replace with build/libs latest"),
        stale_item("MineAstr 0.4.1", r"<AUDIT_ROOT>\mineastr-validation\neoforge-0.4.1\build\libs\mineastr-0.4.1.jar", "old branch; use 0.6.25 current candidate"),
        stale_item("Kaleidoscope End official 1.0.11", r"<AUDIT_ROOT>\official-candidates\kaleidoscope_end-1.0.11-neoforge+mc1.21.1.jar", "official baseline, not migration.7"),
        stale_item("Kaleidoscope Nether official 1.1.4", r"<AUDIT_ROOT>\official-candidates\kaleidoscope_nether-1.1.4-neoforge+mc1.21.1.jar", "not selected because overlay requires base 1.1.2"),
        stale_item("Nautilus legacy i_want_my_nautilus", r"<AUDIT_ROOT>\nautilus-backport-audit\i_want_my_nautilus-0.1-neoforge-1.21.1.jar", "reference implementation; do not co-install with embedded equivalence classes"),
        stale_item("Nautilus legacy platform", r"<AUDIT_ROOT>\nautilus-backport-audit\platform-neoforge-1.21.1-1.3.3.jar", "reference dependency; not required by final equivalence JAR"),
        stale_item("Nautilus legacy VanillaBackport", r"<AUDIT_ROOT>\nautilus-backport-audit\VanillaBackport-neoforge-1.21.1-1.1.7.10.jar", "reference implementation; do not duplicate embedded classes"),
        stale_item("Nautilus alias adapter", r"<AUDIT_ROOT>\nautilus-backport-audit\nautilus-alias-adapter\build\libs\nautilus-alias-adapter-0.1.0-audit.1+mc1.21.1.jar", "experimental and requires legacy i_want_my_nautilus; not final bundle"),
    ]
...
def write_outputs(inventory: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "本清单只记录 `<AUDIT_ROOT>` 中明确指定的 JAR。生产树 `<TRANS_ROOT>\\20260807` 未写入，未启动长时间服务器；未读取世界、日志、玩家资料库或认证数据库。SHA-256 是 JAR 文件摘要，不代表功能门禁已全部关闭。",
        "",
        "## Target",
        "",
    ...
    lines.extend([
        ...
        "- Every canonical/smoke path is constrained to the audit workspace; the generator rejects paths under `<TRANS_ROOT>\\20260807`.",
