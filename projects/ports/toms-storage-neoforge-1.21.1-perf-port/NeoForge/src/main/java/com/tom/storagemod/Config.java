...
public class Config {
	...
	public boolean onlyTrims, runMultithreaded, terminalScanDebug;
	public int invConnectorScanRange;
	public int invConnectorCableRange = 0;
	public int terminalScanThreads;
	public int terminalRecalcCooldownTicks;
	...
	public static class Server {
		...
		public IntValue terminalScanThreads;
		public IntValue terminalRecalcCooldownTicks;
		public IntValue wirelessRange;
		public BooleanValue onlyTrimsConnect, runMultithreaded, terminalScanDebug;
		...
		private Server(ModConfigSpec.Builder builder) {
			...
			terminalScanThreads = builder.comment("Terminal inventory scan worker threads. 0 means automatic. Automatic keeps two CPU threads free for the server and IO.").
					translation("config.toms_storage.terminal_scan_threads").
					defineInRange("terminalScanThreads", 0, 0, 64);

			terminalRecalcCooldownTicks = builder.comment("Minimum ticks between storage terminal item list recalculations. 0 disables throttling.").
					translation("config.toms_storage.terminal_recalc_cooldown_ticks").
					defineInRange("terminalRecalcCooldownTicks", 10, 0, 200);

			terminalScanDebug = builder.comment("Log terminal inventory scan diagnostics. Useful for debugging duplicate counts and slow networks.").
					translation("config.toms_storage.terminal_scan_debug").
					define("terminalScanDebug", false);
	...
	private void load(ModConfig modConfig) {
		if(modConfig.getType() == Type.SERVER) {
			...
			terminalScanThreads = SERVER.terminalScanThreads.get();
			terminalRecalcCooldownTicks = SERVER.terminalRecalcCooldownTicks.get();
			terminalScanDebug = SERVER.terminalScanDebug.getAsBoolean();
