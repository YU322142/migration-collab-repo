...
public class PlatformFilteredInventoryAccess implements IInventoryAccess, IPriority, IItemHandler {
	...
	private InventoryChangeTracker tracker;
	private long filterRevision = Long.MIN_VALUE;
	...
	private InventoryChangeTracker createTracker() {
		return new InventoryChangeTracker(acc.getPlatformHandler()) {
			...
			protected void refreshFilterState(Level level) {
				// Predicates such as PolyFilter may query capabilities. Keep that
				// work on the server thread before an immutable image is published.
	...
	public IInventoryChangeTracker tracker() {
		if (tracker == null) {
			...
			tracker.refresh(acc.getPlatformHandler());
		}
		long revision = filter.getRevision();
	...
	public Object getTrackingKey() {
		return new FilteredTrackingKey(acc.getTrackingKey(), filter.getRouteKey());
	}

	@Override
	public Object getTopologyKey() {
		return new FilteredTrackingKey(acc.getTopologyKey(), filter.getRouteKey());
	}

	@Override
	public Object getRootDeduplicationKey(IInventoryAccess root) {
		return acc.getRootDeduplicationKey(root);
	}

	private static record FilteredTrackingKey(Object access, Object filterRoute) {}
}
