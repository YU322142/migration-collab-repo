...
public class StorageTerminalMenu extends RecipeBookMenu<CraftingInput, CraftingRecipe> implements IDataReceiver, InteractHandler {
	...
	private int lastSlotCount = -1, lastFreeCount = -1;
	private boolean exactSlotCounts;
	...
	public StorageTerminalMenu(MenuType<?> type, int id, Inventory inv, StorageTerminalBlockEntity te) {
		...
		addDataSlot(DataSlots.create(v -> {
			if (!exactSlotCounts)slotCount = v;
		}, () -> te != null ? Math.min(te.getSlotCount(), Short.MAX_VALUE) : -1));
		addDataSlot(DataSlots.create(v -> {
			if (!exactSlotCounts)freeCount = v;
		}, () -> te != null ? Math.min(te.getFreeCount(), Short.MAX_VALUE) : -1));
	}
	...
	public void broadcastChanges() {
		...
		String nextSearch = te.getLastSearch();
		int nextSlotCount = te.getSlotCount();
		int nextFreeCount = te.getFreeCount();
		boolean searchChanged = !nextSearch.equals(search);
		boolean countsChanged = nextSlotCount != lastSlotCount || nextFreeCount != lastFreeCount;
		Consumer<CompoundTag> extraSync = searchChanged || countsChanged ? tag -> {
			if (searchChanged)tag.putString("s", nextSearch);
			if (countsChanged) {
				tag.putInt("tsc", nextSlotCount);
				tag.putInt("tfc", nextFreeCount);
			}
		} : null;
		sync.update(te.getChangeCount(), itemsCount, (ServerPlayer) pinv.player, extraSync);
		search = nextSearch;
		lastSlotCount = nextSlotCount;
		lastFreeCount = nextFreeCount;
	...
	public final void receiveClientNBTPacket(CompoundTag message) {
		...
		if(message.contains("tsc")) {
			slotCount = Math.max(0, message.getInt("tsc"));
			exactSlotCounts = true;
		}
		if(message.contains("tfc")) {
			freeCount = Math.max(0, message.getInt("tfc"));
			exactSlotCounts = true;
		}
		if(onPacket != null)onPacket.run();
	}

	public boolean hasExactSlotCounts() {
		return exactSlotCounts;
	}
