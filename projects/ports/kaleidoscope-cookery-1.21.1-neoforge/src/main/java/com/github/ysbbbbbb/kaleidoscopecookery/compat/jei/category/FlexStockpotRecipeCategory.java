...
public class FlexStockpotRecipeCategory implements IRecipeCategory<RecipeHolder<FlexStockpotRecipe>> {
    public static final RecipeType<RecipeHolder<FlexStockpotRecipe>> TYPE = RecipeType.createRecipeHolderType(
            ResourceLocation.fromNamespaceAndPath(KaleidoscopeCookery.MOD_ID, "flex_stockpot")
    );
    ...
    public static List<RecipeHolder<FlexStockpotRecipe>> getRecipes() {
        ...
        List<RecipeHolder<FlexStockpotRecipe>> stockpotRecipes = Lists.newArrayList();
        stockpotRecipes.addAll(level.getRecipeManager().getAllRecipesFor(ModRecipes.FLEX_STOCKPOT_RECIPE));
        return stockpotRecipes;
    }

    @Override
    public void draw(RecipeHolder<FlexStockpotRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
