...
public class ChoppingBoardBlockEntity extends BaseBlockEntity implements IChoppingBoard {
    ...
    protected void loadAdditional(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        // Older saves use minecraft:air as an explicit sentinel for an empty
        // chopping board. Normalize only the provably empty state; non-empty
        // or partially progressed boards retain their original model data.
        if (EMPTY_MODEL_ID.equals(this.modelId) && this.isProvablyEmptyBoard()) {
            this.modelId = null;
        }
    }
    ...
    public boolean isProvablyEmptyBoard() {
        return this.maxCutCount == 0
                && this.currentCutCount == 0
                && this.currentCutStack.isEmpty()
                && this.result.isEmpty();
    }
}
