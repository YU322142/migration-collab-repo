...
public final class ModSyncAgent {
    ...
    public static void premain(String agentArguments, Instrumentation instrumentation) {
        ...
        log("MCSync " + BuildInfo.VERSION + " 启动前校验开始",
                "MCSync " + BuildInfo.VERSION + " pre-launch verification started");
        try {
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(ModSyncAgent::releaseGuard, "MCSync-lock-release"));
            ...
            if (Boolean.getBoolean("modsync.managedConfigChanged")) {
                ...
                if (Boolean.getBoolean("modsync.disableProcessExit")) {
                    throw new RuntimeException("MCSync 客户端配置已更新；测试模式禁用了正常退出");
                }
                System.exit(0);
            }
            ...
            releaseGuard();
            if (Boolean.getBoolean("modsync.disableProcessExit")) {
                throw new RuntimeException("MCSync 已阻止启动；测试模式禁用了正常退出", failure);
            }
            System.exit(0);
        }
    }
