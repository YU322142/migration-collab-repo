...
public final class SafeSessionHttpClient {
    ...
    /**
     * FlClashCore's TUN mode can answer the Mojang session hostname with a
     * synthetic address from RFC 2544's 198.18.0.0/15 benchmark range.  That
     * address is intentionally not considered public by {@link EndpointPolicy}
     * and must never be accepted for arbitrary caller-supplied endpoints.
     */
    private static final String MOJANG_SESSION_HOST = "sessionserver.mojang.com";
    private static final String MOJANG_HAS_JOINED_PATH = "/session/minecraft/hasJoined";
    ...
    public Response getTrusted(URI uri) throws IOException {
        ...
        if (addresses.length == 0 || addresses.length > EndpointPolicy.MAX_RESOLVED_ADDRESSES
                || Arrays.stream(addresses).anyMatch(address -> !isTrustedAddress(uri, address))) {
    ...
    /**
     * Returns whether a resolved address is acceptable for this trusted
     * request.  Package visibility keeps the rule directly testable without
     * making it part of the public API.
     */
    static boolean isTrustedAddress(URI uri, InetAddress address) {
        return EndpointPolicy.isPublic(address)
                || (isFixedMojangHasJoinedEndpoint(uri) && isFlClashSyntheticAddress(address));
    }

    private static boolean isFixedMojangHasJoinedEndpoint(URI uri) {
        ...
        return MOJANG_HAS_JOINED_PATH.equals(uri.getRawPath());
    }

    private static boolean isFlClashSyntheticAddress(InetAddress address) {
