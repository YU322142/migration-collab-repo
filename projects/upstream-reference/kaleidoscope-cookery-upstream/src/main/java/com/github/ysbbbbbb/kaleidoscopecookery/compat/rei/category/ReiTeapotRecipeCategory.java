...
public class ReiTeapotRecipeCategory implements DisplayCategory<ReiTeapotRecipeCategory.TeapotRecipeDisplay> {
        ...
        registry.getRecipeManager().getAllRecipesFor(ModRecipes.TEAPOT_RECIPE)
                .forEach(r -> {
                    ...
                    List<EntryIngredient> inputs = List.of(EntryIngredient.of(Arrays.stream(r.ingredient().getItems())
                            .map(stack -> EntryStacks.of(stack.copyWithCount(r.ingredientCount())))
                            .toList()));
                    List<EntryIngredient> output = ReiUtil.ofItemStacks(r.result().copyWithCount(TeapotRecipe.OUTPUT_COUNT));

                    registry.add(new TeapotRecipeDisplay(r.getId(), fluidInput, inputs, output, r.time()));
                });
    }
    ...
    public static class TeapotRecipeDisplay extends BasicDisplay {
        ...
        public TeapotRecipeDisplay(ResourceLocation location, List<EntryIngredient> fluidInput,
            ...
            super(List.of(fluidInput.get(0), ingredientInput.get(0)), outputs, Optional.of(location));
