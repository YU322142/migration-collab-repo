...
public class BeehiveTapBehavior implements ITapBehavior {
    @Override
    public boolean isMatch(
        ...
        int honeyLevel = sourceState.getValue(BeehiveBlock.HONEY_LEVEL);
        if (honeyLevel <= 0) {
            return false;
        }
    ...
    public void onEndExtract(
        ...
        if (destinationState.is(ModBlocks.EMPTY_BOTTLE.get())) {
            ...
            int honeyLevel = sourceState.getValue(BeehiveBlock.HONEY_LEVEL);
            level.setBlockAndUpdate(relative, sourceState.setValue(BeehiveBlock.HONEY_LEVEL, honeyLevel - 1));

            level.setBlockAndUpdate(tapPos.below(), ModBlocks.HONEY_BOTTLE.get().defaultBlockState());
